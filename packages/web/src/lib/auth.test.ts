...
describe("buildSignInDecision", () => {
  describe("Google", () => {
    ...
    it("denies an unverified email (boolean false) before any allowlist match", async () => {
      await expect(
        buildSignInDecision({
    ...
    it('denies an unverified email when email_verified is the string "false"', async () => {
      await expect(
        buildSignInDecision({
          account: googleAccount,
          profile: { email_verified: "false" } as unknown as Profile,
    ...
    it("denies when email_verified is absent", async () => {
      await expect(
        buildSignInDecision({
          account: googleAccount,
          profile: {} as Profile,
    ...
    it("allows a verified (boolean true) allowlisted email", async () => {
      await expect(
        buildSignInDecision({
          account: googleAccount,
    ...
    it('allows a verified email when email_verified is the string "true"', async () => {
      await expect(
        buildSignInDecision({
          account: googleAccount,
          profile: { email_verified: "true" } as unknown as Profile,
    ...
    it('accepts a mixed-case "True" string (case-insensitive normalization)', async () => {
      await expect(
        buildSignInDecision({
          account: googleAccount,
          profile: { email_verified: "True" } as unknown as Profile,
    ...
    it("denies a verified email that is not on any allowlist", async () => {
      await expect(
        buildSignInDecision({
          account: googleAccount,
          profile: { email_verified: true } as unknown as Profile,
  ...
  describe("GitHub", () => {
    ...
    it("admits an allowlisted GitHub username without an email_verified check", async () => {
      await expect(
        buildSignInDecision({
    ...
    it("denies a non-allowlisted GitHub user", async () => {
      // Username isn't allowlisted, so the domain allowlist forces a fallback
      // fetch to GitHub's emails endpoint - mock it instead of hitting the network.
    ...
    it("treats an undefined provider as the GitHub path", async () => {
      await expect(
        buildSignInDecision({
          account: undefined,
          profile: { login: "octocat" } as unknown as Profile,
